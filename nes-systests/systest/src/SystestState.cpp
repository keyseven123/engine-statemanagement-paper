...
std::pair<
    ...
    std::unordered_map<std::string, std::pair<std::optional<std::filesystem::path>, uint64_t>>>
optimizeQueryPlanIfErrorFree(const NES::Systest::LoadedQueryPlan& loadedQueryPlan)
{
    std::unordered_map<std::string, std::pair<std::optional<std::filesystem::path>, uint64_t>> sourceNamesToFilepathAndCountForQuery;
...
SystestQuery::SystestQuery(
    ...
    std::unordered_map<std::string, std::pair<std::optional<std::filesystem::path>, uint64_t>> sourceNamesToFilepathAndCount,
...
std::string RunningQuery::getThroughput() const
{
    ...
    double bytesPerSecond = NAN, tuplesPerSecond = NAN;
    ...
    parser.registerOnSystestAttachSourceCallback(
        [&](SystestAttachSource attachSource)
        {
            ...
            if (const auto logicalSource = sourceCatalog->getLogicalSource(attachSource.logicalSourceName))
            {
                ...
                {
                    sourcesToFilePaths[sourceDescriptor.value()] = attachSource.fileDataPath;
                    return;
                }
                throw UnknownSource(
