...
QueryCheckResult checkQuery(const NES::Systest::RunningQuery& runningQuery)
{
    ...
    const auto queryResult = loadQueryResult(runningQuery.systest);
    if (not queryResult.has_value())
    {
        return QueryCheckResult{fmt::format("Failed to load query result for query: ", runningQuery.systest.queryDefinition)};
    }
    ...
    const QuerySchemasAndResults querySchemasAndResults = [&]()
    {
        ...
        auto expectedQueryResult = runningQuery.systest.expectedResultsOrError;
        ...
        return QuerySchemasAndResults(
            ExpectedResultSchema(runningQuery.systest.planInfoOrException.value().sinkOutputSchema),
