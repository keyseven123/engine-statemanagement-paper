...
{
    auto combinedIdxOffset = writeVarSizedData(buffer, value, bufferProvider);
    std::visit(
        [this, combinedIdxOffset](const auto& key)
        {
            ...
            {
                *reinterpret_cast<uint64_t*>(const_cast<uint8_t*>((*this)[key].getAddressPointer())) = combinedIdxOffset;
            }
            else
            {
                PRECONDITION(
    ...
    return std::visit(
        [this](const auto& key)
        {
            if constexpr (
                ...
                || std::is_convertible_v<std::decay_t<decltype(key)>, std::string>)
            {
                const auto index = (*this)[key].template read<uint64_t>();
                return readVarSizedDataAsString(this->buffer, index);
            }
            else
            {
                PRECONDITION(
...
bool DynamicTuple::operator==(const DynamicTuple& other) const
{
    ...
    return std::ranges::all_of(
        ...
        [this, &other](const auto& field)
        {
            ...
            {
                const auto thisString
                    = readVarSizedDataAsString(buffer, thisDynamicField.template read<TupleBuffer::NestedTupleBufferKey>());
                const auto otherString
                    = readVarSizedDataAsString(other.buffer, otherDynamicField.template read<TupleBuffer::NestedTupleBufferKey>());
