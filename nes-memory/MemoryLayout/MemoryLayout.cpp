...
void copyVarSizedAndIncrementMetaData(
    Memory::TupleBuffer& childBuffer, const uint64_t childBufferOffset, const int8_t* varSizedValue, const uint32_t varSizedValueLength)
{
...
uint64_t writeVarSizedData(const Memory::TupleBuffer& buffer, const std::string_view value, Memory::AbstractBufferProvider& bufferProvider)
{
    ...
    *reinterpret_cast<uint32_t*>(str.data()) = value.length();
    ...
    return storeAssociatedVarSizedValue(&buffer, &bufferProvider, reinterpret_cast<const int8_t*>(str.c_str()), str.length());
}
