...
public:
    ...
    static QueryDecomposer::DecomposedLogicalPlan planQueryFromTestYAML(const std::filesystem::path& pathToQueryConfig)
    {
        ...
        CLI::YAMLBinder::BoundLogicalPlan boundPlan = CLI::YAMLBinder::from(std::move(queryConfig)).bind();
        CLI::LegacyOptimizer::OptimizedLogicalPlan optimizedPlan = CLI::LegacyOptimizer::optimize(std::move(boundPlan));
        OperatorPlacer::PlacedLogicalPlan placedPlan = OperatorPlacer::from(std::move(optimizedPlan)).place();
        QueryDecomposer::DecomposedLogicalPlan decomposedPlan = QueryDecomposer::from(std::move(placedPlan)).decompose();
...
TEST_F(DistributedPlanningTest, BasicPlacementSingleNode)
{
    ...
    const LogicalPlan localPlan = plan.at("0.0.0.0:50051");
...
TEST_F(DistributedPlanningTest, BasicPlacementTwoNodes)
{
    ...
    const auto sourceNodePlan = plan.at("source-node-grpc");
    ...
    const auto sinkNodePlan = plan.at("sink-node-grpc");
...
TEST_F(DistributedPlanningTest, JoinPlacementWithOneSelection)
{
    ...
    const auto sourceNode0Plan = decomposedPlan.at("source-node0-grpc");
    ...
    const auto sourceNode1Plan = decomposedPlan.at("source-node1-grpc");
    ...
    const auto sinkNodePlan = decomposedPlan.at("sink-node-grpc");
...
TEST_F(DistributedPlanningTest, PlacementWithThreeNodes)
{
    ...
    const auto plan0 = decomposedPlan.at("sink-node-grpc");
    ...
    const auto plan1 = decomposedPlan.at("source-node0-grpc");
    ...
    const auto plan2 = decomposedPlan.at("source-node1-grpc");
...
TEST_F(DistributedPlanningTest, JoinPlacementWithLimitedCapacityOnTwoNodes)
{
    ...
    const auto sinkPlan = decomposedPlan.at("sink-node-grpc");
    ...
    const auto sourcePlan = decomposedPlan.at("source-node-grpc");
...
TEST_F(DistributedPlanningTest, BridgePlacement)
{
    ...
    const auto intermediatePlan = decomposedPlan.at("intermediate-node-grpc");
...
TEST_F(DistributedPlanningTest, LongBridgePlacement)
{
    ...
    const auto intermediatePlan0 = decomposedPlan.at("intermediate-node0-grpc");
    ...
    const auto intermediatePlan1 = decomposedPlan.at("intermediate-node0-grpc");
