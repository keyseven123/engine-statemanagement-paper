    ...
    const std::vector<std::tuple<std::optional<TopologyGraph::NodeId>, TopologyGraph::Path, TopologyGraph::Path>>& candidates) const
{
    for (const auto& [lca, path1, path2] : candidates)
    {
        ...
        {
            if (ctx.nodeCatalog.getCapacityFor(*it) > 0)
            {
...
OperatorPlacer::OperatorPlacer(CLI::LegacyOptimizer::OptimizedLogicalPlan&& inputPlan) : plan{std::move(inputPlan.plan)}, ctx{inputPlan.ctx}
{
}

OperatorPlacer OperatorPlacer::from(CLI::LegacyOptimizer::OptimizedLogicalPlan&& inputPlan)
{
    return OperatorPlacer{std::move(inputPlan)};
}
...
OperatorPlacer::PlacedLogicalPlan OperatorPlacer::place() &&
{
    pinSources();
    pinSinks();
    PRECONDITION(plan.getRootOperators().size() == 1, "BottomUpPlacement requires queries to have a single sink");
    PRECONDITION(plan.getRootOperators().front().tryGet<SinkLogicalOperator>().has_value(), "The root of the plan needs to be the sink");
    PRECONDITION(hasTrait<PlacementTrait>(plan.getRootOperators().front().getTraitSet()), "The sink needs to be pinned before placement");
    const auto& sources = getLeafOperators(plan);
    PRECONDITION(
        std::ranges::all_of(
    ...
    return PlacedLogicalPlan{
        .plan = LogicalPlan{plan.getQueryId(), {this->placeOperatorRecursive(plan.getRootOperators().front())}, plan.getOriginalSql()},
        .ctx = std::move(ctx)};
}
...
OperatorPlacer::placeBinaryOperator(const LogicalOperator& op, const LogicalOperator& leftChild, const LogicalOperator& rightChild)
{
    ...
    const auto candidates = ctx.topology.findLCA(
        leftChildPlacement.value(),
        rightChildPlacement.value(),
        getPlacementFor(plan.getRootOperators().front()).value(),
    ...
    ctx.nodeCatalog.consumeCapacity(*selectedNode, 1);
...
LogicalOperator OperatorPlacer::placeUnaryOperator(const LogicalOperator& op, const LogicalOperator& child)
{
    ...
    if (const auto& operatorTargetNode
        = findFirstNodeWithCapacity(childPlacement.value(), getPlacementFor(plan.getRootOperators().front()).value());
        operatorTargetNode.has_value())
    {
        /// Reduce capacity on the selected node and place the operator
        ctx.nodeCatalog.consumeCapacity(*operatorTargetNode, 1);
...
{
    const auto paths = ctx.topology.findPaths(start, nextPlacedOperatorNode, TopologyGraph::Downstream);
    if (paths.empty())
    {
        throw NoPathFound();
    }
    ...
    {
        if (ctx.nodeCatalog.hasCapacity(nodeId))
        {
            return {nodeId};
        }
    }
...
void OperatorPlacer::pinSources()
{
    LogicalPlan withPlacedSources = plan;
    for (const auto& source : getOperatorByType<SourceDescriptorLogicalOperator>(plan))
    {
    ...
    plan = std::move(withPlacedSources);
}

void OperatorPlacer::pinSinks()
{
    const auto sink = getOperatorByType<SinkLogicalOperator>(plan).front();
    const auto& placement = ctx.sinkCatalog.at(sink.sinkName);
    plan = *replaceOperator(plan, sink.id, sink.withTraitSet({PlacementTrait{placement}}));
}
}
