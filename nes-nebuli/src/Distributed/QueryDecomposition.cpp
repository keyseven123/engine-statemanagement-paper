...
LogicalOperator QueryDecomposer::createNetworkSource(const Channel& channel) const
{
    ...
    const auto logicalSource
        = ctx.sourceCatalog
              ->addLogicalSource(fmt::format("Net:{}-{}", channel.upstreamOp, channel.downstreamOp), upstreamOp.getOutputSchema())
              .value();

    auto descriptor = ctx.sourceCatalog
                          ->addPhysicalSource(
                              logicalSource,
                              *getPlacementFor(downstreamOp),
                              "Network",
                              ...
                              ParserConfig{.parserType = "Raw", .tupleDelimiter = "", .fieldDelimiter = ""})
                          .value();
    ...
    return SourceDescriptorLogicalOperator{std::move(descriptor)}
        ...
        .withTraitSet({PlacementTrait{getPlacementFor(downstreamOp).value()}});
}

LogicalOperator QueryDecomposer::createNetworkSink(const Channel& channel) const
{
    const auto upstreamOp = findOperatorById(channel.upstreamOp);
    const auto downstreamOp = findOperatorById(channel.upstreamOp);

    auto networkSink = SinkLogicalOperator{fmt::format("Net:{}-{}", channel.upstreamOp, channel.downstreamOp)};
    networkSink.sinkDescriptor = std::make_shared<Sinks::SinkDescriptor>(
        "Network",
        Configurations::DescriptorConfig::Config{
            {"type", "Network"}, {"channel", channel.id}, {"connection", getPlacementFor(downstreamOp).value()}},
        false);

    return networkSink.withInputOriginIds({upstreamOp.getOutputOriginIds()})
        .withOutputOriginIds(upstreamOp.getOutputOriginIds())
        .withTraitSet({PlacementTrait{getPlacementFor(downstreamOp).value()}})
        .withInferredSchema({upstreamOp.getOutputSchema()});
}

QueryDecomposer::QueryDecomposer(OperatorPlacer::PlacedLogicalPlan&& placedPlan)
    : inputPlan{std::move(placedPlan.plan)}, ctx{std::move(placedPlan.ctx)}
{
...
QueryDecomposer QueryDecomposer::from(OperatorPlacer::PlacedLogicalPlan&& placedPlan)
{
    return QueryDecomposer(std::move(placedPlan));
}

QueryDecomposer::DecomposedLogicalPlan QueryDecomposer::decompose() &&
{
    PRECONDITION(inputPlan.getRootOperators().size() == 1, "Requires single root operator");
    PRECONDITION(
        std::ranges::all_of(
            BFSRange(inputPlan.getRootOperators().front()), [](const auto& op) { return hasTrait<PlacementTrait>(op.getTraitSet()); }),
        "Query decomposition requires placement of all operators");

    distribute(); /// Step 1: distribute query plan among workers based on placement traits of operators
    for (const auto& [node, plan] : planByNode)
    {
        NES_DEBUG("Plan fragment for node [{}] after plan distribution: \n{}", node, plan);
    }
    connect(); /// Step 2: connect plan fragments on nodes with network sources/sinks to enable sending of intermediate results
    DecomposedLogicalPlan decomposedPlan;
    for (auto& [node, plan] : planByNode)
    {
        decomposedPlan.emplace(ctx.nodeCatalog.getGrpcAddressFor(node), std::move(plan));
    }
    return decomposedPlan;
}

/// This function takes the input logical plan and assumes that all operators have been assigned to a node by attaching a placement trait to them.
/// We do a BFS over the plan and operate on parent-child operator pairs until all pairs have been processed.
void QueryDecomposer::distribute()
{
    std::queue<std::pair<LogicalOperator, LogicalOperator>> bfsQueue;
    /// Initialize with root operator placed on its node
    ...
    while (not bfsQueue.empty())
    {
        ...
        INVARIANT(planByNode.contains(parentPlacement), "Node where the parent is assigned should already have a plan fragment");
    ...
    const TopologyGraph::NodeId& childNode)
{
    ...
    const auto [path] = ctx.topology.findPaths(parentNode, childNode, TopologyGraph::Upstream).front();
...
{
    /// In both cases, we only operate on the roots --> child is on a different node, but also the lowest level we have processed so far
    /// Therefore, the childOp can only be a root
    /// Here, we relax on the constraint that each plan only has a single root (sink), because we need to slice the plan arbitrarily depending on placement
    const auto opWithoutChildren = op.withChildren({});
    const auto node = getPlacementFor(op).value();
    if (auto [it, inserted]
        = planByNode.try_emplace(node, LogicalPlan{inputPlan.getQueryId(), {opWithoutChildren}, inputPlan.getOriginalSql()});
        not inserted)
    {
        auto& fragment = planByNode.at(node);
        auto updatedRoots = fragment.getRootOperators();
        updatedRoots.push_back(opWithoutChildren);
        fragment.setRootOperators(updatedRoots);
    }
}

void QueryDecomposer::connect()
{
    for (const auto& channel : channels)
    {
        /// Use replaceOperator to keep the
