...
YAMLBinder::YAMLBinder(QueryConfig&& config)
    : plan(AntlrSQLQueryParser::createLogicalQueryPlanFromSQLString(config.query))
    , ctx{
          .topology = TopologyGraph::from(config.workerNodes),
...
void YAMLBinder::bindRegisterLogicalSources(const std::vector<LogicalSource>& unboundSources)
{
    ...
    for (const auto& [logicalSourceName, schemaFields] : unboundSources)
    {
        ...
        if (not ctx.sourceCatalog->addLogicalSource(logicalSourceName, schema).has_value())
        {
...
void YAMLBinder::bindRegisterPhysicalSources(const std::vector<PhysicalSource>& unboundSources)
{
    ...
    {
        auto logicalSource = ctx.sourceCatalog->getLogicalSource(logicalSourceName);
        ...
        const auto sourceDescriptorOpt = ctx.sourceCatalog->addPhysicalSource(
...
void YAMLBinder::bindValidateSink(const Sink& unboundSink)
{
    ...
    if (sinkOperator->sinkName == unboundSink.name)
    {
        ...
        const auto sinkDescriptor = std::make_shared<Sinks::SinkDescriptor>(unboundSink.type, std::move(validatedSinkConfig), false);
    ...
    ctx.sinkCatalog[sinkOperator->sinkName] = unboundSink.host;
}

YAMLBinder::BoundLogicalPlan YAMLBinder::bind() &&
{
    ...
    bindValidateSink(queryConfig.sink);
    return BoundLogicalPlan{.plan = std::move(plan), .ctx = std::move(ctx)};
}
