...
class QueryDecomposer
{
    ...
    explicit QueryDecomposer(OperatorPlacer::PlacedLogicalPlan&& placedPlan);

public:
    static QueryDecomposer from(OperatorPlacer::PlacedLogicalPlan&& placedPlan);
    ...
    using DecomposedLogicalPlan = std::unordered_map<CLI::GrpcAddr, LogicalPlan>;
...
private:
    ...
    LogicalOperator createNetworkSink(const Channel& channel) const;
    LogicalOperator createNetworkSource(const Channel& channel) const;
