...
QueryPlanBuilder::TestPlanCtrl QueryPlanBuilder::build(QueryId queryId, std::shared_ptr<Memory::BufferManager> bm) &&
{
    ...
    std::unordered_map<identifier_t, std::shared_ptr<TestPipelineOperatorHandlerController>> const pipelineOperatorHandlerCtrls;
...
std::unique_ptr<Runtime::InstantiatedQueryPlan> TestingHarness::addNewQuery(QueryPlanBuilder&& builder)
{
    ...
    auto [plan, pSourceIds, pPipelineIds, pSourceCtrls, pSinkCtrls, pPipelineCtrls, pOperatorHandlerCtrls, pStages]
        = std::move(builder).build(queryId, bm);
