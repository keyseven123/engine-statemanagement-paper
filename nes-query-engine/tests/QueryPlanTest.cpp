...
struct EmittedTask
{
    ...
    {
        std::unique_lock const lock(mutex);
        for (auto it = arguments.begin(); it != arguments.end() /* not hoisted */; /* no increment */)
        {
...
TEST_F(QueryPlanTest, RunningQueryNodeSetup)
{
    ...
    EXPECT_TRUE(terminations->handle(stage1Ptr));
    EXPECT_TRUE(terminations->handle(stage2Ptr));
}
