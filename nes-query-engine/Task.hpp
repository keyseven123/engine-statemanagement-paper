...
{
    return std::visit([](auto& specificTask) { return specificTask.complete(); }, task);
}

inline auto failTask(const Task& task, Exception exception)
{
    return std::visit([&](auto& specificTask) { return specificTask.fail(std::move(exception)); }, task);
}
