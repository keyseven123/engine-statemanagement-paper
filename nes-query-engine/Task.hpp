...
{
public:
    using onComplete = TaskCallback::onComplete;
    using onSuccess = TaskCallback::onSuccess;
    using onFailure = TaskCallback::onFailure;
...
{
    WorkTask(QueryId queryId, PipelineId pipelineId, std::weak_ptr<RunningQueryPlanNode> pipeline, Memory::TupleBuffer buf, TaskCallback callback)
