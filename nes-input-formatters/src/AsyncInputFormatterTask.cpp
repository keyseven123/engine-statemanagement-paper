    ...
    const std::vector<size_t>& fieldSizes)
{
    ...
    const std::string_view firstPartialTuple = std::string_view(
        firstBuffer.buffer.getBuffer<const char>() + (firstBuffer.offsetOfLastTupleDelimiter + sizeOfTupleDelimiter),
...
AsyncInputFormatterTask::AsyncInputFormatterTask(
    ...
    , numberOfFieldsInSchema(this->schema->getFieldCount())
    , inputFormatter(std::move(inputFormatter))
    , sizeOfTupleInBytes(this->schema->getSchemaSizeInBytes())
{
    ...
    for (const std::shared_ptr<AttributeField>& field : *this->schema)
    {
    ...
    Runtime::Execution::PipelineExecutionContext& pec) const
{
    ...
    const auto numberOfBuffersToFill
        = calculateNumberOfRequiredBuffers(totalNumberOfTuplesInRawBuffer, numberOfTuplesInFirstFormattedBuffer, numberOfTuplesPerBuffer);
    ...
    const Memory::TupleBuffer& inputTupleBuffer, Runtime::Execution::PipelineExecutionContext& pipelineExecutionContext)
{
    ...
    auto fieldOffsets
        = FieldOffsetIterator(schema->getFieldCount(), sizeOfFormattedTupleBufferInBytes, pipelineExecutionContext.getBufferManager());
